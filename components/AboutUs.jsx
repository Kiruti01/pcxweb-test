        ...
        <div className="max-w-220 mx-auto flex flex-col gap-6 max-md:gap-4">
          {storyCards.map((card) => (
            ...
            >
              ...
              >
                <h2 className="story-card-title font-inter text-[22px] max-md:text-lg font-bold leading-[1.3] text-[#13161A] mb-3 max-md:mb-2 transition-colors duration-220 group-hover:text-[#1D5EFF]">
                  {card.title}
                </h2>
                <p className="font-mono text-[13px] max-md:text-xs font-normal leading-6 max-md:leading-5 text-[#3E4953] m-0">
                  {card.body}
                </p>
                {card.bullets && (
                  <div className="mt-4 max-md:mt-3">
                    <p className="font-mono text-[13px] max-md:text-xs text-[#3E4953] mb-2">
                      We've:
                    </p>
                    <ul className="list-disc list-inside flex flex-col gap-1">
                      {card.bullets.map((b, i) => (
                        <li
                          key={i}
                          className="story-bullet font-mono text-[13px] max-md:text-xs text-[#3E4953] leading-6 max-md:leading-5 transition-all duration-200 hover:text-[#1D5EFF] hover:translate-x-1"
                        >
                          {b}
                        </li>
                      ))}
              ...
              {/* Image */}
              <div
                className={`w-64 max-md:w-full shrink-0 max-md:max-w-60 max-md:mx-auto transition-all duration-300 group-hover:scale-104 group-hover:-translate-y-1 group-hover:drop-shadow-[0_8px_24px_rgba(29,94,255,0.15)] ${card.imageLeft ? "order-1 max-md:order-2" : "order-2"}`}
              >
                <img
                  src={card.id === 3 && dark ? "/from_incubation_dark.svg" : card.image}
                  alt={card.title}
